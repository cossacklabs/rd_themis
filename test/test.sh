...
testThemisMessageSet() {
    ...
    res=`redis-cli -x "rd_themis.mset test_key $key test_data"`
    assertEquals "OK" "$res"
}

testThemisMessageGet() {
    res=`echo -ne '\x52\x45\x43\x32\x00\x00\x00\x2d\xc7\xa8\xca\x7a\x00\xc3\xb5\xd1\xad\x51\x37\x30\x8f\x45\xe6\x5e\x54\xdf\x2b\x7a\x45\xbc\x85\x08\xe8\xcc\x3b\xc9\x48\x1b\x63\x1a\xe8\x12\x8b\x39\x74' | redis-cli -x rd_themis.mget test_key `
    assertEquals "test_data" "$res"
}
